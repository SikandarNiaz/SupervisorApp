...
@Component({
  selector: 'app-upload-distribution-assets',
  templateUrl: './upload-distribution-assets.component.html',
  styleUrls: ['./upload-distribution-assets.component.css'],
  providers: [
...
export class UploadDistributionAssetsComponent  implements OnInit {
...
//   }
  ...
  uploadData(post) {
    ...
    formData.append("fileType", "distribution");
    if (
      ...
      this.loadingData = true;
      this.httpService.uploadTargetVsAchievement(formData).subscribe((data) => {
        if (data) {
          debugger;
          this.response = data;
