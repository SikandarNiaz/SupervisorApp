...
export class CeShoptargetService {
  ...
  getShopDataForUpdateTarget(obj) {
    const urlencoded = this.UrlEncodeMaker(obj);
    const url = this.ip + "shopTarget"; // ----> SurveyShopListController
    return this.http.post(url, urlencoded, this.httpOptions);
  }
  ...
  getShopByZoneAndRegion(zoneId, regionId) {
    const filter = JSON.stringify({
      act: 15,
      zoneId: zoneId,
      regionId: regionId,
