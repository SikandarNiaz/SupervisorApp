...
@Component({
  selector: 'app-ndn-vo-live-tracking',
  templateUrl: './ndn-vo-live-tracking-component.html',
  styleUrls: ['./ndn-vo-live-tracking-component.scss']
})
export class NdnVoLiveTrackingComponent implements OnInit {
  ...
  getSupervisorData() {
    ...
    const obj = {
      ...
      surveyorId: parseInt(localStorage.getItem("surveyorId"))
    };
  ...
  goToEvaluation(shop) {
    ...
    window.open(
      `${
        environment.hash
      }dashboard/virtual_view/list/ndn-routerTracker?surveyorId=${
        shop.surveyorId
  ...
  getDetail() {
   ...
   console.log("Selected Surveyor Filter:", this.selectedSurveyorFilter);
   console.log("Selected Supervisor Filter:", this.selectedSupervisorFilter);
   console.log("Start Date:", moment(this.startDate).format("YYYY-MM-DD"));
   console.log("Selected Region ID:", this.selectedRegion.id);
   console.log("Selected Zone ID:", this.selectedZone.id);

   this.httpService.getDetailList(obj).subscribe((data: any) => {
     if (data) {
