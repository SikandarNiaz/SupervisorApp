...
@Component({
  selector: "app-transmed-vo-live-tracking",
  templateUrl: "./transmed-vo-live-tracking.component.html",
  styleUrls: ["./transmed-vo-live-tracking.component.scss"],
})
export class TransmedVoLiveTrackingComponent implements OnInit {
  ...
  goToEvaluation(shop) {
    ...
    window.open(
      `${
        environment.hash
      }dashboard/virtual_view/list/transmed-routerTracking?surveyorId=${
        shop.surveyorId
      }&startDate=${this.modifyDate(
        shop.visit_date
      )}&userType=${localStorage.getItem("user_id")}`,
