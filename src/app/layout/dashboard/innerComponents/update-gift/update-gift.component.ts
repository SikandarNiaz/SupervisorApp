...
export class UpdateGiftComponent implements OnInit {
  ...
  showupdateSuperAttendenceModal(shop): void {
    debugger;
    this.selectedRemark=shop.prize_id;
    this.shopData=shop;
  ...
  getDashboardData() {
    ...
    this.loading = true;
    // this.getTabsData();
    this.httpService.getHelpersList(obj).subscribe((data: any) => {
      this.loadingData = false;
      if (data) {
...
;
  ...
  getGifts() {
    ...
    this.httpService.getHelperGifts().subscribe(
      (data) => {
        const res: any = data;
        if (res) {
          this.giftList = res;
  ...
  updateGift(shop) {
   ...
   console.log("prizeID: ", shop.prize_id, "shopID: ", shop.shop_id,"surveyorID: ", shop.surveyor_id,"count: ", shop.count);
    this.httpService.updateHelpersGifts(obj).subscribe((data: any) => {
      debugger;
      if (data.success == true) {
