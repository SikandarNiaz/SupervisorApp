...
@Component({
  selector: 'app-show-stock-data',
  templateUrl: './show-stock-data.component.html',
  styleUrls: ['./show-stock-data.component.scss']
})
export class ShowStockDataComponent implements OnInit {
   // ip = environment.ip;
  ...
  getData() {
    ...
    this.httpService.getStockData(obj).subscribe(
      (data) => {
        // console.log(data);
        this.tableData = data;
  ...
  getSupervisor() {
      this.loading = true;
       ...
       this.httpService.getBaSupervisorsListWithNameCode().subscribe(
        (data) => {
          const res: any = data;
          if (res) {
