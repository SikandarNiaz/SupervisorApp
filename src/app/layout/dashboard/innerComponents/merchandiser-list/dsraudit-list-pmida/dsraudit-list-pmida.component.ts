...
@Component({
  selector: 'app-dsr-audit-list-pmida',
  templateUrl: './dsraudit-list-pmida.component.html',
  styleUrls: ['./dsraudit-list-pmida.component.css']
})
export class DsrAuditListPmidaComponent implements OnInit {title = '';
  ...
  getMerchandiserList() {
    ...
    this.httpService.getMerchandiserListForEvaluation(obj).subscribe((data: any) => {
      ...
      if (data) {
        ...
        console.log("this.merchandiserList",this.merchandiserList);
        this.loading = false;
        this.loadingData = false;
      }
    });
  ...
  gotoNewPage(item) {
    ;
    // window.open(
    //   `${environment.hash}dashboard/evaluation/list/home?surveyId=${item.survey_id}&type=${"DsrAudit"}`,
    //   '_blank'
    // );
    window.open(`${environment.hash}dashboard/evaluation/list/details/${item.survey_id}/${item.m_code}/${'dsrAudit'}`, '_blank');
