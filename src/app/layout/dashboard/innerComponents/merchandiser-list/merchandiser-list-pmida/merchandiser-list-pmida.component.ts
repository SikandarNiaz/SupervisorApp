...
@Component({
  selector: 'app-merchandiser-list-pmida',
  templateUrl: './merchandiser-list-pmida.component.html',
  styleUrls: ['./merchandiser-list-pmida.component.css']
})
export class MerchandiserListPmidaComponent implements OnInit {title = '';
  ...
  gotoNewPage(item) {
    ;
    window.open(
      `${environment.hash}dashboard/evaluation/list/home?surveyorId=${item.id}&startDate=${this.modifyDate(
        this.startDate
      )}&endDate=${this.modifyDate(this.endDate)} &userType=${this.userTypeId}&type=${'marketAudit'}`,
      '_blank'
    );
  }
