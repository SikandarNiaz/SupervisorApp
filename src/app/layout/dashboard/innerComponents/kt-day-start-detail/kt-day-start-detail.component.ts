...
@Component({
  selector: 'app-kt-day-start-detail',
  templateUrl: './kt-day-start-detail.component.html',
  styleUrls: ['./kt-day-start-detail.component.scss']
})
export class KTDayStartData implements OnInit {
   // ip = environment.ip;
  ...
  getData() {
    ...
    this.httpService.getKTDayStartData(obj).subscribe(
      (data) => {
        // console.log(data);
        this.tableData = data;
  ...
  getSupervisor() {
    ...
    gotoNewPage(item) {
      ...
      const urlWithParams = `${environment.hash}dashboard/kt-attendance-evaluation?${queryParams.toString()}`;
