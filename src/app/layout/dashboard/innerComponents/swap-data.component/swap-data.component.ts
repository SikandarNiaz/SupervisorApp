...
export class ShowSwapDataComponent implements OnInit {
   // ip = environment.ip;
  ...
  getData() {
    ...
    this.httpService.getSwapkData(obj).subscribe(
      (data) => {
        // console.log(data);
        this.tableData = data;
  ...
  getRegion() {
    ...
    goToEvaluation(shop) {
      ...
      window.open(
        `${
          environment.hash
        }dashboard/section_ten?surveyId=${
          shop.surveyId
        }`,
        "_blank"
      );
    ...
    goToUpdateTargetPage(shop) {
      window.open(
        `${environment.hash}dashboard/section_detail?surveyId=${shop.Survey_Id}`,
        "_blank"
      );
