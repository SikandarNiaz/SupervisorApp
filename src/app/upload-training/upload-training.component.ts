...
export class UploadTrainingComponent implements OnInit {
  ...
  uploadTrainingPDF() {
    ...
    formData.append('title', this.fileToUpload.name.split('.').slice(0, -1).join('.')); // Remove extension
    formData.append('description', this.fileToUpload.name.split('.').slice(0, -1).join('.'));
